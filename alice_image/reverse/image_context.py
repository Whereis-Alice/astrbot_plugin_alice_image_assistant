...
class ImageContextManager:
    ...
    def _prepare_session(self, session: SessionImages) -> None:
        ...
        if self.ttl_seconds > 0:
            ...
            if removed > 0:
                logger.debug(f"[AliceImageContext] 已清理 {removed} 张过期图片")
    ...
    def add_image(
        ...
        with self._lock:
            ...
            if info:
                logger.debug(
                    f"[AliceImageContext] 添加图片: {info.image_id} {url[:50]}..."
                )
...
def init_image_context_manager(
    ...
    logger.info(
        "[AliceImageContext] 已初始化，隔离模式: %s, max_images: %s, ttl_seconds: %s, max_sessions: %s",
        isolation_mode,
        max_images,
        ttl_seconds,
        max_sessions,
