...
def set_user_agent(user_agent: str | None) -> None:
    ...
    if user_agent and user_agent.strip():
        ...
        logger.info(f"[AliceImageReverse] 已设置 User-Agent: {_user_agent[:50]}...")
    else:
        _user_agent = None
        logger.debug("[AliceImageReverse] 未设置自定义 User-Agent，将使用默认值")
...
def set_allow_image_upload(allow: bool) -> None:
    ...
    logger.info(f"[AliceImageReverse] 已设置图床上传策略: {status}")
...
def set_allow_local_file_access(allow: bool) -> None:
    ...
    status = "允许" if allow else "禁止"
    logger.info(f"[AliceImageReverse] 已设置本地文件访问策略: {status}")
...
async def upload_image(image_bytes: bytes) -> str | None:
    ...
    if len(image_bytes) > 200 * 1024 * 1024:
        logger.warning("[AliceImageReverse] 图片过大，超过 200MB 限制")
        return None
...
async def get_http_image_url(source: str) -> str | None:
    ...
    if not is_image_upload_allowed():
        logger.warning(
            "[AliceImageReverse] 图片上传已被禁用，仅支持 HTTP URL 图片搜图。"
