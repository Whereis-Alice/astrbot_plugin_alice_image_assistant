...
class AliceReverseController:
    ...
    def _config_to_dict(config: object) -> dict:
        ...
        try:
            ...
            try:
                ...
                logger.warning("[AliceImageReverse] 无法解析配置对象，使用空配置")
                return {}
    ...
    def _init_strategies(self) -> None:
        ...
        if enable_saucenao and sauce_nao_key:
            ...
            logger.info(
                f"[AliceImageReverse] 已加载 SauceNAO 策略 (相似度阈值: {saucenao_threshold}%)"
            )
        elif enable_saucenao:
            logger.warning("[AliceImageReverse] SauceNAO API Key 未配置，跳过该策略")
        else:
            logger.info("[AliceImageReverse] SauceNAO 策略已禁用")
        ...
        if enable_google_lens and serpapi_keys and isinstance(serpapi_keys, list):
            ...
            logger.warning(
                "[AliceImageReverse] SerpAPI Keys 未配置，跳过 Google Lens 策略"
            )
        else:
            logger.info("[AliceImageReverse] Google Lens 策略已禁用")
        ...
        if enable_ascii2d and ascii2d_session_id:
            ...
            logger.warning("[AliceImageReverse] Ascii2d session_id 未配置，跳过该策略")
        else:
        ...
        if len(self.strategies) == 0:
            logger.warning("[AliceImageReverse] 没有可用策略；以图搜图会保持关闭状态。")
    ...
    async def _run_command_search(
        ...
        logger.info(
            f"[AliceImageReverse] 收到命令搜图请求，策略: {strategy_names or '全部'}"
        )
    ...
    async def _get_image_from_reply(
        ...
        if bot:
            try:
                ...
                logger.debug(f"[AliceImageReverse] 获取回复消息失败: {e}")
    ...
    async def _send_search_results(
        ...
        if display.get("forward_message_enabled", True) and is_aiocqhttp_platform(
            ...
            try:
                ...
                logger.warning(
                    f"[AliceImageReverse] 合并转发发送失败，降级为普通消息: {e}"
                )

        try:
            await self._send_normal_msg(event, items)
        except Exception as e:
            logger.error(f"[AliceImageReverse] 普通消息发送失败，降级为纯文本: {e}")
    ...
    async def _send_forward_msg(
        ...
        for idx, item in enumerate(items, start=1):
            node = Node(
                ...
                content=AliceReverseController._build_forward_content(idx, item),
            )
            nodes.append(node)
        ...
        try:
            ...
            logger.warning(
                f"[AliceImageReverse] 转发消息发送失败，尝试仅移除可疑图片重试: {e}"
            )
            ...
            removed_count = 0
            for idx, item in enumerate(items, start=1):
                drop_image = AliceReverseController._is_suspicious_forward_image(item)
                ...
                retry_nodes.append(
                    Node(
                        name="爱丽丝图片助手",
                        uin=str(event.get_self_id() or "0"),
                        content=AliceReverseController._build_forward_content(
            ...
            if removed_count == 0:
                logger.warning(
                    "[AliceImageReverse] 未识别到可疑图片来源，保持原异常交由上层降级"
                )
                raise

            logger.info(
                f"[AliceImageReverse] 已移除 {removed_count} 张可疑来源图片，保留其他图片后重试转发"
            )
