...
class RandomSearchService:
    def __init__(
        ...
        *,
        ...
        self.tag_search_enabled = bool(tag_search_enabled)
        self.ranking_enabled = bool(ranking_enabled)
    ...
    def start(self):
        """启动后台任务"""
        if not self.tag_search_enabled and not self.ranking_enabled:
            logger.info("Pixiv 随机搜索与随机排行榜均已禁用，不启动调度器。")
            return
