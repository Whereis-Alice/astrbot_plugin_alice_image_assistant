...
async def _convert_ugoira_to_gif(zip_data, metadata, safe_title, illust_id):
    ...
    temp_dir = None
    try:
        # 解析绝对路径，避免阻塞事件循环，也避免执行到非预期的同名程序。
        ...
        def extract_frames() -> dict[str, Path]:
            with zipfile.ZipFile(zip_path, "r") as zip_ref:
                zip_ref.extractall(temp_dir)
            return {
                path.name: path for path in Path(temp_dir).iterdir() if path.is_file()
            }

        available_frames = await asyncio.to_thread(extract_frames)
        ...
        if not await asyncio.to_thread(output_gif.exists):
            logger.error("Pixiv 插件：GIF文件未生成")
            return None

        # 读取GIF文件为字节数据
        try:
            async with aiofiles.open(output_gif, "rb") as f:
                gif_data = await f.read()
